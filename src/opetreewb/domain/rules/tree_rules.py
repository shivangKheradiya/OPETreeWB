...
class TreeRules:
    ...
    def can_create_node(parent_node, element_type) -> RuleResult:
        ...
        if not schema:
            Reporter.error(f"[TreeRules] No schema found for {elmType}")

            return RuleResult.deny(f"No schema found for {elmType}")

        allowed = schema.allowed_children()

        if element_type not in allowed:
            Reporter.error(f"[TreeRules] {element_type} not allowed under {elmType}")
            return RuleResult.deny(f"{element_type} not allowed under {elmType}")

        Reporter.info(f"[TreeRules] {element_type} allowed under {elmType}")
