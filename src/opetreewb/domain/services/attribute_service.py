...
class AttributeService:
    ...
    def update_attribute(self, node, attribute_name, data_id, new_value):
        ...
        if not result.allowed:
            Reporter.error(f"[AttributeService] Update denied: {result.reason}")
            return False
        ...
        if data_id is None:
            ...
            def local_op():
                ...
                node.attributes[attribute_name] = AttributeValue(
                    data_id=new_data_id, value=new_value
                )

                Reporter.info(f"[LOCAL] attribute created (data_id={new_data_id})")
        ...
        # -------------------------
            ...
            def local_op():
                ...
                Reporter.info(f"[LOCAL] attribute updated (data_id={data_id})")
        ...
        Reporter.success(
            f"[AttributeService] Update allowed (data_id={data_id}, value={new_value})"
        )
